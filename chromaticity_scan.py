...
if __name__ == "__main__":
    ...
    parser.add_argument('--job_name', action='store', metavar='JOB_NAME', type=str, default='chromaticity_scan',
                        help='Name of the job and associated .our and .err files. Defaults to "chromaticity_scan"')
    ...
    parser.add_argument('--max_chromaticity', action='store', metavar='MAX_CHROM', type=float, default = 3.0, 
                        help="Maximal chromaticity value for chromaticity scan. Defaults to 3.0." )
    parser.add_argument('--min_chromaticity', action='store', metavar='MIN_CHROM', type=float, default = 0.01, 
                        help="Maximal chromaticity value for chromaticity scan. Defaults to 0.01." )
    parser.add_argument('--n_scan_points', action='store', metavar='N_SCAN_POINTS', type=int, default = 30,
                        help='Number of chromaticity value to be scanned. Each point is a separate job! Defaults to 30.')
    args = parser.parse_args()
    if args.sub_mode == 'ccrt':
        for  i, Qp_y  in enumerate(np.linspace(args.min_chromaticity, args.max_chromaticity, args.n_scan_points)):
