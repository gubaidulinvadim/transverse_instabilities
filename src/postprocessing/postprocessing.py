...
def plot_offset(ax, m, mp, std, n_macroparticles, n_turns, n_bin, bunch_current, Qp_x, Qp_y, n_bunches=416, n_sampling=1, **matplotlib_kwargs):
    turns = np.linspace(0, len(m)/n_bunches*n_sampling, len(m)) 
    ax.plot(turns, m/std, **matplotlib_kwargs)
    min_level = 0.05
    signal = np.sqrt(np.trim_zeros(m, trim='b')**2+(BETA_Y_SMOOTH*np.trim_zeros(mp, trim='b'))**2)/std
...
def post_single(n_macroparticles=1e6, n_turns=5e4, n_bin=100, bunch_current=1.2e-3, Qp_x = 1.6, Qp_y=1.6, ID_state='open', Zlong='False', cavity='False'):
    ...
    fig, ax = plt.subplots(1, 1)
    risetime = plot_offset(ax, m[2,:], m[3,:], std[2,0], n_macroparticles, n_turns, n_bin, bunch_current, Qp_x, Qp_y, n_bunches=1, n_sampling=1)
