...
from PyHEADTAIL.impedances.wakes import ResistiveWall, WakeField, WakeSource
...
from PyHEADTAIL.radiation.radiation import (SynchrotronRadiationLongitudinal,
                                            SynchrotronRadiationTransverse)
...
def run_ht(n_turns=50000,
    ...
    PHI_RF = np.arcsin(
        U_LOSS / V_RF) if (GAMMA**-2 -
                           GAMMA_T**-2) < 0 else pi + np.arcsin(U_LOSS / V_RF)
    ...
    long_map = RFSystems(CIRCUMFERENCE, [
        H_RF,
    ], [
        V_RF,
    ], [
    ...
    wake_slicer = slicing.UniformBinSlicer(n_slices=n_wake_slices,
                                           z_cuts=z_cuts)
    ...
    print('Transverse radiation damping times in turns {:.0f}, {:.0f}'.format(
        TAU_X * OMEGA_REV / (2 * np.pi), TAU_Y * OMEGA_REV / (2 * np.pi)))
    print('Longitudinal radiation damping times in turns {:.0f}'.format(
        TAU_Z * OMEGA_REV / (2 * np.pi)))
    synchr_rad = SynchrotronRadiationTransverse(
        eq_emit_x=bunch.epsn_x(),
    ...
    synchr_rad_long = SynchrotronRadiationLongitudinal(
        eq_sig_dp=SIGMA_DP,
        damping_time_z_turns=TAU_Z * OMEGA_REV / (2 * np.pi),
        E_loss_eV=e * U_LOSS)
    ...
    chroma = Chromaticity(Qp_x=[Qp_x], Qp_y=[Qp_y])
