...
def run_mbtrack2(n_turns=50000, n_macroparticles=int(1e5), n_bin=100, bunch_current=1e-3, Qp_x=1.6, Qp_y=1.6, ID_state='open', include_Zlong=False, harmonic_cavity='False'):
    ...
    monitor_filename = FOLDER+'monitors(n_mp={:.1e},n_turns={:.1e},n_bin={:},bunch_current={:.1e},Qp_x={:.2f},Qp_y={:.2f},cavity={:})'.format(
        n_macroparticles, n_turns, n_bin, bunch_current, Qp_x, Qp_y, harmonic_cavity
    )
    ...
    if harmonic_cavity == 'True':
        V_RF = 1.2753602707269827e6
        theta_main = 1.2753602707269827
        rf = RFCavity(ring, m=1, Vc=V_RF, theta=theta_main)
        V_RF_harmonic = 4.292124905802067e5
        theta_harmonic = -1.7210480862530364
        harmonic_rf = RFCavity(
            ring, m=4, Vc=V_RF_harmonic, theta=theta_harmonic)
    else:
        rf = RFCavity(ring, m=1, Vc=V_RF, theta=np.pi-np.arcsin(ring.U0/V_RF))
