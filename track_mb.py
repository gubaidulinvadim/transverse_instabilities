...
def run_mbtrack2(n_turns=50000, n_macroparticles=int(1e5), n_bin=100, bunch_current=1e-3, Qp_x=1.6, Qp_y=1.6, ID_state='open', include_Zlong=False, n_turns_wake=1):
    ring2 = v2366_v2(IDs=ID_state, load_lattice=False)
    particle = Electron()
    chro = [Qp_x, Qp_y]
    ...
    beam_monitor = BeamMonitor(ring.h, save_every=1, buffer_size=500,
                               file_name=monitor_filename, total_size=n_turns, mpi_mode=True)
